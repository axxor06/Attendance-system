...
export default function LoginPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      navigate(MANAGEMENT_ROLES.includes(user.role) ? '/hod' : `/${user.role}`);
    } catch (err) {
  ...
  return (
    <AuthLayout title="Sign in to continue" subtitle="Use your college email or register number. Your role and workspace are detected automatically.">
          ...
          <Input
            ...
            endAdornment={(
              <button
                ...
                className="rounded-lg p-1 text-slate transition-colors hover:bg-ink/5 hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber"
      ...
      >
        <p>
          New student?{' '}
          <Link to="/register" className="font-semibold text-ink hover:text-amber">Create an account</Link>
        </p>
        <p className="mt-2 text-xs text-slate/75">
          Waiting for approval?{' '}
          <Link to="/request-registration" className="font-semibold text-ink hover:text-amber">Request registration</Link>
          {' · '}
          <Link to="/check-request-status" className="font-semibold text-ink hover:text-amber">Check status</Link>
        </p>
      </motion.div>
    </AuthLayout>
  );
