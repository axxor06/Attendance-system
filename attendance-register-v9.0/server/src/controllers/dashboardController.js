...
import { User, Subject, Attendance, ActivityLog, Class, Department, PeriodTemplate } from '../models/index.js';
...
export const getHodDashboard = asyncHandler(async (req, res) => {
  ...
  const isScopedHod = req.user.role === ROLES.HOD;
  const classIds = isScopedHod ? await Class.distinct('_id', { department: req.user.department || null }) : null;
  const attendanceScope = isScopedHod ? { class: { $in: classIds || [] } } : {};
  const userScope = isScopedHod ? { department: req.user.department || null } : {};
  const activityActorScope = isScopedHod ? { actor: { $in: await User.distinct('_id', userScope) } } : {};

  const [totalStudents, totalFaculty, totalDepartments, totalClasses] = await Promise.all([
    User.countDocuments({ ...userScope, role: ROLES.STUDENT, isActive: true }),
    User.countDocuments({ ...userScope, role: ROLES.FACULTY, isActive: true }),
    isScopedHod ? 1 : Department.countDocuments({ isActive: true }),
  ...
  const [[todayAgg], [monthAgg], lowAttendanceStudents, recentActivity, trend] = await Promise.all([
    ...
    getLowAttendanceStudents({ threshold: LOW_ATTENDANCE_THRESHOLD, classIds: isScopedHod ? classIds : undefined }),
...
export const getFacultyDashboard = asyncHandler(async (req, res) => {
  ...
  const [subjects, recentAttendance, todayMarkedCount] = await Promise.all([
    Subject.find({ faculty: facultyId, isActive: true })
...
export const getStudentDashboard = asyncHandler(async (req, res) => {
  ...
  const [overall, subjectWise, monthly, recent, todayAttendance, template] = await Promise.all([
    ...
    PeriodTemplate.findOne({ dayOfWeek: todayName, isActive: true }),
  ]);
  ...
  if (template) {
    ...
    timetable = template.periods.map(period => ({
      order: period.order,
      name: period.name,
      kind: period.kind,
