...
export const exportSubjectReport = asyncHandler(async (req, res) => {
  ...
  const studentFilter = departmentScope
    ? departmentScope.departmentId
      ? {
          $or: [
            { department: departmentScope.departmentId },
            { class: { $in: departmentScope.classIds } },
          ],
        }
      : { _id: null }
    : undefined;

  const rows = await getSubjectRoster({ subjectId, from, to, studentFilter });
...
export const exportClassMonthlyReport = asyncHandler(async (req, res) => {
  ...
  const students = await User.find({ class: classId, role: 'student' }).select('name registerNumber').limit(MAX_REPORT_ROWS);
