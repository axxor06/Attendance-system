    ...
    <AuthLayout title="Sign in to continue" subtitle="Use your college email or register number. Your role and workspace are detected automatically.">
      ...
      >
          ...
          <Input
            ...
            endAdornment={(
              <button
                ...
                className="rounded-lg p-1 text-slate transition-colors hover:bg-indigo/5 hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
        ...
        <motion.div variants={staggerItem} className="flex items-center justify-between gap-4">
          ...
          <Link to="/forgot-password" className="shrink-0 text-sm font-semibold text-ink transition-colors hover:text-accent">Forgot password?</Link>
        </motion.div>
        ...
        {error && (
          <motion.div
            ...
            className="rounded-md border border-clay/30 bg-clay-light px-4 py-3 text-sm leading-6 text-clay"
            role="alert"
          >
            {error}
          </motion.div>
      ...
      >
        <p>
          New applicant?{' '}
          <Link to="/request-registration?role=student" className="font-semibold text-ink hover:text-accent">Request student access</Link>
          {' · '}
          <Link to="/request-registration?role=faculty" className="font-semibold text-ink hover:text-accent">Request faculty access</Link>
        </p>
        <p className="mt-2 text-xs text-slate/75">
          Already requested access?{' '}
          <Link to="/check-request-status" className="font-semibold text-ink hover:text-accent">Check request status</Link>
        </p>
      </motion.div>
    </AuthLayout>
  );
