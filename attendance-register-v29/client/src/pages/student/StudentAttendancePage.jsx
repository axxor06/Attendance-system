...
export default function StudentAttendancePage() {
  ...
  async function handleDownload(format) {
    ...
    try {
      ...
      toast.error(getFriendlyError(err, 'Could not generate your attendance report.'));
    } finally {
      setDownloading(null);
    }
  }
    ...
    <motion.div className="flex flex-col gap-6" {...fadeUp}>
      ...
      <Card className="overflow-hidden">
        <div className="flex flex-wrap items-center gap-3 border-b border-ink/8 px-5 py-4">
          ...
          <div className="directory-search w-full sm:w-56">
            <Search size={16} className="directory-search-icon" aria-hidden="true" />
            <input
              value={search}
              onChange={e => setSearch(e.target.value)}
              placeholder="Search subject"
              aria-label="Search subject"
              className="field directory-search-input min-h-11 py-2 text-sm"
            />
          </div>
          <select
            value={filterStatus}
            ...
            className="field min-h-11 w-full bg-paper px-3 py-2 text-sm sm:w-auto"
          >
          ...
          {(search || filterStatus !== 'all') && (
              ...
              className="rounded-lg px-2.5 py-2 text-xs font-semibold text-slate transition-colors hover:bg-paper-dim hover:text-ink">
              Clear
            </button>
          )}
        </div>
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate">
                      {formatCalendarDate(r.date, 'MMM d, yyyy')} · {r.periodName}
                    </p>
                  </div>
                </div>
