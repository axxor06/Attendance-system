...
| Security middleware | Helmet, explicit CORS allowlist, cookie Origin/Fetch Metadata guards, request IDs, Mongo sanitization, strict body contracts, express-validator, endpoint-specific rate limits |
...
`npm run seed` populates the default multi-department demo dataset. `npm run seed:polytechnic` is a separate, alternate dataset modeling a single real institution (6 departments, 6 semesters); run one or the other against a given database, not both.
