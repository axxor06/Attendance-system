...
// Mirrors the fallback already used in classController.js and
// ClassesPage.jsx for real (non-seed) departments: an explicit
// semesterCount wins, otherwise diploma programs default to 6 and
// everything else to 8. Kept as one function so the seed can't drift
// from what the live app actually does.
function effectiveSemesterCount(dept) {
  return dept.semesterCount || (dept.programLevel === 'diploma' ? 6 : 8);
}
...
function subjectsForClass(dept, semNumber) {
  ...
  const pool = [
    ...
    semNumber >= 7
      ...
      : 'Open Elective',
    semNumber === effectiveSemesterCount(dept)
      ? 'Project Work'
      : 'Mini Project',
  ];
  ...
  for (const definition of departmentDefs) {
    ...
    if (!department) {
      ...
      department = {
        ...
        programLevel: definition.programLevel || 'degree',
        semesterCount: effectiveSemesterCount(definition),
        isActive: true,
        createdBy: hodId,
        createdAt: now,
        updatedAt: now,
...
async function ensureSemesters(hodId, now) {
  ...
  // Global semester slots must cover the longest configured program.
  // Departments below this length simply never reference the extra slots
  // (see classController.js/ClassesPage.jsx, which already filter available
  // semesters to each department's own semesterCount) - this mirrors that
  // existing, already-correct behavior rather than introducing a new one.
  const maxSemesterCount = departmentDefs.reduce(
    (max, dept) => Math.max(max, effectiveSemesterCount(dept)),
    8
  );

  for (let number = 1; number <= maxSemesterCount; number++) {
...
async function ensureClasses(
  ...
  for (const dept of departmentDefs) {
    ...
    const semesterCount = effectiveSemesterCount(dept);

    for (let semNumber = 1; semNumber <= semesterCount; semNumber++) {
