...
import { sendOtpEmail, sendPasswordChangedEmail, sendEmailChangeRequestedNotice } from '../utils/email.js';
...
async function loadSafeAuthUser(userId) {
  const user = await User.findById(userId)
    .select('+passwordResetRequired +pendingEmail')
...
async function sendEmailChangeRequestedNoticeSafely(req, { to, name, newEmail }) {
  try {
    await sendEmailChangeRequestedNotice({ to, name, newEmail });
  } catch (error) {
    // The pending change is already committed; SMTP delivery of the
    // heads-up notice is a non-critical side effect and must not turn a
    // successful request into a 500.
    console.warn(`[${req.id || 'no-request-id'}] Email-change notice delivery failed (${error?.code || 'unknown'}).`);
  }
}
...
export const login = asyncHandler(async (req, res) => {
  ...
  }).select('+password +failedLoginAttempts +loginFailureWindowStartedAt +loginLockedUntil +passwordResetRequired +deviceBindingHash +roleModelVersion');
...
export const updateMe = asyncHandler(async (req, res) => {
  const { name, phone, dateOfBirth, designation, qualification, admissionYear, avatarUrl, employeeId, department } = req.body;
...
export const requestEmailChange = asyncHandler(async (req, res) => {
  const normalizedEmail = String(req.body.newEmail).trim().toLowerCase();
  const user = await User.findById(req.user._id);
  if (!user) throw ApiError.notFound('Account not found.');

  if (normalizedEmail === user.email) throw ApiError.badRequest('That is already your current email address.');
  ...
  const otp = await createOtp(normalizedEmail, OTP_PURPOSE.EMAIL_CHANGE);
  await sendOtpEmail({ to: normalizedEmail, name: user.name, otp, purpose: OTP_PURPOSE.EMAIL_CHANGE });
  await sendEmailChangeRequestedNoticeSafely(req, { to: user.email, name: user.name, newEmail: normalizedEmail });
  ...
  await logActivity({
    ...
    targetType: 'User',
    targetId: user._id,
    description: 'Requested an email address change (pending verification)',
    ipAddress: getClientIp(req),
    requestId: req.id,
  });

  return sendResponse(res, 200, `A verification code was sent to ${normalizedEmail}. Enter it to confirm the change.`, { pendingEmail: normalizedEmail });
});
...
export const confirmEmailChange = asyncHandler(async (req, res) => {
  ...
  if (!user) throw ApiError.notFound('Account not found.');
  if (!user.pendingEmail) throw ApiError.badRequest('There is no pending email change to confirm.');

  const result = await verifyOtp(user.pendingEmail, OTP_PURPOSE.EMAIL_CHANGE, otp);
  if (!result.valid) throw ApiError.badRequest(result.reason);

  // Re-check uniqueness: another account could have claimed this address
  // during the verification window.
  const duplicate = await User.exists({ email: user.pendingEmail, _id: { $ne: user._id } });
  if (duplicate) {
  ...
  await logActivity({
    ...
    targetType: 'User',
    targetId: user._id,
    description: 'Confirmed an email address change',
    ipAddress: getClientIp(req),
    requestId: req.id,
    oldValue: { email: previousEmail },
  ...
  await notifyUser({
    ...
    message: `Your login email is now ${user.email}.`,
  });

  const safeUser = await loadSafeAuthUser(user._id);
  return sendResponse(res, 200, 'Email address updated successfully.', { user: safeUser });
});
...
export const cancelEmailChange = asyncHandler(async (req, res) => {
  ...
  user.pendingEmail = null;
  await user.save();
  return sendResponse(res, 200, 'Pending email change cancelled.');
});
