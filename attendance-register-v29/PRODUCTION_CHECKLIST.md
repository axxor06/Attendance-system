...
The current sandbox has mock-backed browser evidence for success, empty, and API-error states. The v28 static suite also covers strict body contracts, Fetch Metadata cookie protection, pinned JWT algorithms, fail-closed production configuration, canonical legacy-role migration guards, shared academic identifier validation, and atomic registration rejection. Source and contract tests also cover exact Faculty period scoping, pending-link filtering, request ordering, and duplicate-read sharing. MongoDB, Redis, SMTP, deployed HTTPS, real seed execution, concurrent database writes, raw slot-ID persistence, exact availability, unchanged saves, available-Faculty saves, occupied-Faculty HTTP 409 behavior, messaging relationships, text persistence, BOLA attempts, read/unread state, notification delivery, and real authentication sessions remain **UNVERIFIED** until this staging checklist is completed.
