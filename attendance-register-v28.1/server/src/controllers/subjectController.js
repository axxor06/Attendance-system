...
import { ACTIVITY_ACTION, canonicalRole, roleValues, ROLES } from '../config/constants.js';
import { assertSubjectAccess, applyDepartmentScope, isSameId } from '../utils/authorization.js';
import { paginationMeta, parsePagination } from '../utils/pagination.js';
import { getFacultySubjectScope } from '../services/timetableService.js';
...
async function validateSubjectRelationships({ actor, department, semester, classId, faculty = [], students = [] }) {
  ...
  if (faculty.length > 0) {
    const facultyCount = await User.countDocuments({ _id: { $in: faculty }, role: { $in: roleValues(ROLES.ADMIN) }, isActive: true });
    if (facultyCount !== new Set(faculty.map(String)).size) throw ApiError.badRequest('Every subject Faculty member must be active.');
  }
  if (students.length > 0) {
    const studentCount = await User.countDocuments({ _id: { $in: students }, role: { $in: roleValues(ROLES.USER) }, isActive: true, class: classDoc._id });
...
export const getSubjects = asyncHandler(async (req, res) => {
  ...
  let filter = actorRole === ROLES.ADMIN
    ? (department ? { department } : {})
    : applyDepartmentScope(req, department ? { department } : {});
  if (semester) filter.semester = semester;
  if (classId) filter.class = classId;
  if (facultyId) filter.faculty = facultyId;
  if (actorRole === ROLES.ADMIN) {
    filter.$and = [...(filter.$and || []), await getFacultySubjectScope(req.user._id)];
    delete filter.faculty;
  }
  if (actorRole === ROLES.USER) filter.class = req.user.class || null;
  if (search) {
...
export const getMySubjects = asyncHandler(async (req, res) => {
  const pagination = parsePagination(req.query, { defaultLimit: 50, maxLimit: 100 });
  const facultyScope = await getFacultySubjectScope(req.user._id);
  const filter = { isActive: true, ...facultyScope };
  const [subjects, total] = await Promise.all([
