...
import { getAdminResetPasswordTtlMs } from '../config/security.js';
import {
...
function generateTemporaryCredential(length = 16) {
...
export const resetUserPassword = asyncHandler(async (req, res) => {
  const user = await User.findOne(await applyUserScope(req, { _id: req.params.id })).select('+passwordResetRequired +passwordResetExpiresAt');
  await assertManageableUser(req.user, user);

  // The credential is returned only once to the authorized HOD, stored only as a
  // bcrypt hash, expires quickly, and forces a real password change at sign-in.
  const temporaryPassword = req.body.newPassword || generateTemporaryCredential();
  const passwordExpiresAt = new Date(Date.now() + getAdminResetPasswordTtlMs());
  user.password = temporaryPassword;
  user.passwordResetRequired = true;
  user.passwordResetExpiresAt = passwordExpiresAt;
  Object.assign(user, getLoginFailureReset());
  user.tokenVersion = Number(user.tokenVersion || 0) + 1;
  await user.save();
  await revokeUserSessions(user._id);
  await notifyUser({ userId: user._id, type: NOTIFICATION_TYPE.PASSWORD_CHANGED, title: 'Temporary password issued', message: 'An authorized administrator issued a temporary password. Sign in before it expires, then change it immediately.' });
  await logActivity({ actorId: req.user._id, action: ACTIVITY_ACTION.PASSWORD_RESET, targetType: 'User', targetId: user._id, description: `Issued expiring temporary password for ${user.name}`, ipAddress: req.ip, requestId: req.id });
  return sendResponse(res, 200, 'Temporary password reset successfully. It expires soon and must be changed after sign-in.', { resetCredential: temporaryPassword, passwordIsTemporary: true, passwordExpiresAt: passwordExpiresAt.toISOString() });
});
