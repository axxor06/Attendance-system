...
export const protect = asyncHandler(async (req, res, next) => {
  ...
  const user = await User.findById(decoded.id).select('+deviceBindingHash +roleModelVersion +passwordResetRequired +passwordResetExpiresAt');
  ...
  req.user = user;
  const isPasswordChangeRoute = req.method === 'POST'
    && req.baseUrl.endsWith('/auth')
    && req.path === '/change-password';
  if (user.passwordResetRequired && !isPasswordChangeRoute) {
    throw ApiError.forbidden('Your temporary password must be changed before accessing the system.');
  }
  next();
});
