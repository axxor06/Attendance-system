...
async function start() {
  ...
  server.listen(PORT, HOST, () => {
    console.info(`[Server] Running in ${process.env.NODE_ENV || 'development'} mode on ${HOST}:${PORT}`);
  });
  ...
  const shutdown = (signal) => {
    ...
    shuttingDown = true;
    console.info(`[Server] ${signal} received; draining connections.`);
    ...
    server.close(async () => {
      try {
        ...
        console.error('[Server] Shutdown failed', {
          category: 'SERVER_SHUTDOWN_FAILURE',
          errorName: error?.name || 'Error',
          errorCode: typeof error?.code === 'string' ? error.code.slice(0, 80) : undefined,
        });
        process.exit(1);
      }
    });
...
start().catch((err) => {
  console.error('[Server] Startup failed', {
    ...
    errorName: err?.name || 'Error',
    errorCode: typeof err?.code === 'string' ? err.code.slice(0, 80) : 'STARTUP_ERROR',
  });
  process.exit(1);
});
