...
const { assertSecurityConfiguration, getAdminResetPasswordTtlMs } = await import('../../server/src/config/security.js');
const { errorHandler } = await import('../../server/src/middleware/errorHandler.js');
const { isTemporaryPasswordExpired } = await import('../../server/src/controllers/authController.js');
...
test('administrative reset credentials expire and require a password change', () => {
  const userSource = fs.readFileSync(new URL('../../server/src/models/User.js', import.meta.url), 'utf8');
  ...
  assert.equal(User.schema.path('passwordResetExpiresAt').options.select, false);
  assert.equal(isTemporaryPasswordExpired({ passwordResetRequired: true, passwordResetExpiresAt: new Date('2026-08-20T00:00:00.000Z') }, new Date('2026-08-20T00:00:01.000Z')), true);
  assert.equal(isTemporaryPasswordExpired({ passwordResetRequired: true, passwordResetExpiresAt: new Date('2026-08-20T00:00:02.000Z') }, new Date('2026-08-20T00:00:01.000Z')), false);
  assert.equal(isTemporaryPasswordExpired({ passwordResetRequired: false, passwordResetExpiresAt: new Date('2026-08-20T00:00:00.000Z') }, new Date('2026-08-20T00:00:01.000Z')), false);
  assert.equal(getAdminResetPasswordTtlMs({ ADMIN_RESET_PASSWORD_TTL_MS: '900000' }), 900000);
  assert.equal(getAdminResetPasswordTtlMs({ ADMIN_RESET_PASSWORD_TTL_MS: '1' }), 900000);
  assert.match(userController, /passwordResetRequired = true/);
  assert.match(userController, /passwordResetExpiresAt = passwordExpiresAt/);
  assert.match(userController, /passwordIsTemporary: true/);
  assert.match(authController, /This temporary password has expired/);
  assert.match(authController, /passwordResetExpiresAt = null/);
  assert.match(userSource, /passwordResetExpiresAt:/);
});
...
test('photo uploads and all-error responses remain bounded and safe', () => {
  ...
  assert.match(errorHandler, /path: req\.path/);
  assert.doesNotMatch(errorHandler, /path: req\.originalUrl/);
...
test('administrator reset wiring uses a one-time expiring temporary credential response', () => {
  ...
  assert.match(resetFunction, /generateTemporaryCredential\(\)/);
  assert.match(resetFunction, /user\.password = temporaryPassword/);
  assert.match(resetFunction, /passwordResetRequired = true/);
  assert.match(resetFunction, /passwordResetExpiresAt = passwordExpiresAt/);
  assert.match(resetFunction, /resetCredential: temporaryPassword/);
  assert.match(resetFunction, /passwordIsTemporary: true/);
  assert.match(resetFunction, /passwordExpiresAt/);
...
test('administrator reset creates an expiring credential with one-time authorized display', () => {
  const controller = fs.readFileSync(new URL('../../server/src/controllers/userController.js', import.meta.url), 'utf8');
  const userSource = fs.readFileSync(new URL('../../server/src/models/User.js', import.meta.url), 'utf8');
  assert.match(controller, /randomInt/);
  assert.match(controller, /generateTemporaryCredential/);
  assert.match(controller, /const temporaryPassword = req\.body\.newPassword \|\| generateTemporaryCredential\(\)/);
  assert.match(controller, /user\.password = temporaryPassword/);
  assert.match(controller, /passwordResetRequired = true/);
  assert.match(controller, /passwordResetExpiresAt = passwordExpiresAt/);
  assert.match(controller, /passwordIsTemporary: true/);
  assert.match(controller, /resetCredential: temporaryPassword/);
  assert.match(controller, /passwordExpiresAt/);
  assert.match(userSource, /bcrypt\.hash\(this\.password/);
...
test('production configuration rejects invalid cookie and HTTP lifecycle overrides', () => {
  const valid = {
    NODE_ENV: 'production',
    JWT_ACCESS_SECRET: 'A1!access-secret-with-enough-entropy-2026',
    JWT_REFRESH_SECRET: 'B2@refresh-secret-with-enough-entropy-2026',
    ...
    ADMIN_RESET_PASSWORD_TTL_MS: '900000',
  };
  assert.doesNotThrow(() => assertSecurityConfiguration(valid));
  assert.throws(() => assertSecurityConfiguration({ ...valid, HTTP_REQUEST_TIMEOUT_MS: '250' }), /HTTP_REQUEST_TIMEOUT_MS/);
  assert.throws(() => assertSecurityConfiguration({ ...valid, REFRESH_COOKIE_SAMESITE: 'invalid' }), /REFRESH_COOKIE_SAMESITE/);
  assert.throws(() => assertSecurityConfiguration({ ...valid, REFRESH_COOKIE_SAMESITE: 'none', REFRESH_COOKIE_SECURE: 'false' }), /REFRESH_COOKIE_SECURE/);
  assert.throws(() => assertSecurityConfiguration({ ...valid, ADMIN_RESET_PASSWORD_TTL_MS: '299999' }), /ADMIN_RESET_PASSWORD_TTL_MS/);
  assert.throws(() => assertSecurityConfiguration({ ...valid, ADMIN_RESET_PASSWORD_TTL_MS: '86400001' }), /ADMIN_RESET_PASSWORD_TTL_MS/);
...
test('global error handling keeps duplicate and malformed-request responses safe and actionable', () => {
  function capture(error) {
    ...
    errorHandler(error, { method: 'POST', originalUrl: '/api/test', id: 'request-test-1', get: () => null }, res, () => {});
    return response;
  }

  const duplicate = capture(Object.assign(new Error('duplicate key'), { code: 11000, keyValue: { email: 'hidden@example.test' } }));
  assert.equal(duplicate.statusCode, 409);
  assert.equal(duplicate.payload.error.code, 'EMAIL_ALREADY_EXISTS');
  assert.match(duplicate.payload.error.message, /already exists/i);
  assert.doesNotMatch(duplicate.payload.error.message, /hidden@example\.test/);

  const malformed = new SyntaxError('Unexpected token');
  malformed.status = 400;
  malformed.body = '{';
  const invalidJson = capture(malformed);
  assert.equal(invalidJson.statusCode, 400);
  assert.equal(invalidJson.payload.error.code, 'INVALID_JSON');
  assert.match(invalidJson.payload.error.message, /invalid JSON/i);

  const oversized = capture(Object.assign(new Error('body too large'), { type: 'entity.too.large' }));
  assert.equal(oversized.statusCode, 413);
  assert.equal(oversized.payload.error.code, 'PAYLOAD_TOO_LARGE');
});


test('HTTP boundary includes restrictive browser capability policy and response compression', async () => {
  ...
  assert.match(appSource, /compression\(\{ threshold: 1024 \}\)/);
});


test('temporary-password accounts are server-gated until password change', () => {
  const authSource = fs.readFileSync(new URL('../../server/src/middleware/auth.js', import.meta.url), 'utf8');
  assert.match(authSource, /\+passwordResetRequired \+passwordResetExpiresAt/);
  assert.match(authSource, /isPasswordChangeRoute/);
  assert.match(authSource, /Your temporary password must be changed before accessing the system/);
  assert.match(authSource, /req\.baseUrl\.endsWith\('\/auth'\)/);
  assert.match(authSource, /req\.path === '\/change-password'/);
});


test('operational provider failure logs omit raw error messages', () => {
  const emailSource = fs.readFileSync(new URL('../../server/src/utils/email.js', import.meta.url), 'utf8');
  const activitySource = fs.readFileSync(new URL('../../server/src/services/activityLogService.js', import.meta.url), 'utf8');
  const attendanceSource = fs.readFileSync(new URL('../../server/src/controllers/attendanceController.js', import.meta.url), 'utf8');
  const dbSource = fs.readFileSync(new URL('../../server/src/config/db.js', import.meta.url), 'utf8');
  assert.match(emailSource, /SMTP_VERIFY_FAILURE/);
  assert.doesNotMatch(emailSource, /err\.message/);
  assert.match(activitySource, /ACTIVITY_LOG_WRITE_FAILURE/);
  assert.doesNotMatch(activitySource, /err\.message/);
  assert.match(attendanceSource, /LOW_ATTENDANCE_EMAIL_FAILURE/);
  assert.match(attendanceSource, /LOW_ATTENDANCE_BACKGROUND_FAILURE/);
  assert.doesNotMatch(attendanceSource, /err\.message/);
  assert.match(dbSource, /DATABASE_RUNTIME_FAILURE/);
  assert.doesNotMatch(dbSource, /err\.message/);
});
