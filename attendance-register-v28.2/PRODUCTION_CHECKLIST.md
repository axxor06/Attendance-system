...
| HOD reset | Authorized HOD receives a cryptographically random temporary password once; it is not emailed, logged, or stored in plaintext; it expires within the configured TTL, active sessions are revoked, and the Student must complete a first-login password change. The API blocks other protected resources until that change succeeds. | [ ] |
...
The current sandbox has mock-backed browser evidence for success, empty, and API-error states. The v28.1 static suite also covers strict body contracts, Fetch Metadata cookie protection, pinned JWT algorithms, fail-closed production configuration including cookie, HTTP lifecycle, and administrative reset TTL validation, canonical legacy-role migration guards, shared academic identifier validation, atomic registration rejection, structured security headers/logging, safe duplicate/malformed-body errors, server-side temporary-password first-login gating, actionable network-error mapping, and visibility-aware Messages polling. Source and contract tests also cover exact Faculty period scoping, pending-link filtering, request ordering, and duplicate-read sharing. MongoDB, Redis, SMTP, deployed HTTPS, real seed execution, concurrent database writes, raw slot-ID persistence, exact availability, unchanged saves, available-Faculty saves, occupied-Faculty HTTP 409 behavior, messaging relationships, text persistence, BOLA attempts, read/unread state, notification delivery, and real authentication sessions remain **UNVERIFIED** until this staging checklist is completed.
