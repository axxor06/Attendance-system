...
export default function AccountManagementPage({ managedRole, tutorsOnly = false }) {
  ...
  async function handleResetPassword() {
    ...
    try {
      ...
      const passwordExpiresAt = data?.data?.passwordExpiresAt;
      if (!resetCredential || !passwordExpiresAt) throw new Error('The reset response did not include a temporary password and expiry.');
      setResetResult({ email: resetTarget.email, resetCredential, passwordExpiresAt });
  ...
  return (
    <div className="space-y-7">
      ...
      <Modal isOpen={Boolean(resetTarget)} onClose={closeReset} title="Reset password">
        {resetResult ? <div className="space-y-4"><div className="notice-success"><CheckCircle2 size={19} /><div><p className="font-semibold text-ink">Temporary password generated</p><p className="mt-1 text-sm text-slate">This credential is shown once for {resetResult.email}. Existing sessions were revoked. The temporary password expires at <strong className="text-ink">{new Date(resetResult.passwordExpiresAt).toLocaleString()}</strong> and must be changed after sign-in.</p></div></div><div className="border border-line bg-paper p-4"><p className="eyebrow">One-time display credential</p><div className="mt-2 flex items-center gap-2 border border-line bg-surface px-3 py-2"><code className="min-w-0 flex-1 break-all font-mono text-sm text-ink">{showResetCredential ? resetResult.resetCredential : '••••••••••••••••'}</code><button type="button" className="icon-button" onClick={() => setShowResetCredential((value) => !value)} aria-label={showResetCredential ? 'Hide temporary password' : 'Show temporary password'}>{showResetCredential ? <EyeOff size={16} /> : <Eye size={16} />}</button><button type="button" className="icon-button" onClick={copyResetCredential} aria-label="Copy temporary password"><Copy size={16} /></button></div></div><p className="notice-caution"><strong>Shown once.</strong> It is not stored in plaintext and cannot be viewed after this window closes.</p><Button className="w-full" onClick={closeReset}>Done</Button></div> : <div className="space-y-5"><p className="text-sm leading-6 text-slate">Generate a temporary password for <strong className="text-ink">{resetTarget?.name}</strong>? All active sessions will be revoked.</p><div className="flex justify-end gap-2"><Button variant="ghost" onClick={closeReset} disabled={isResetting}>Cancel</Button><Button variant="amber" onClick={handleResetPassword} isLoading={isResetting}>Generate temporary password</Button></div></div>}
      </Modal>
