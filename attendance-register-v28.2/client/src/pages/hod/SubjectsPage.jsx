...
export default function SubjectsPage() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      setLoadError(getFriendlyError(err, 'Subjects, classes, and Faculty options could not be loaded.'));
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(getFriendlyError(err, 'Could not create subject'));
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.error(getFriendlyError(err, 'Could not delete subject'));
    } finally {
      setIsDeleting(false);
    }
  }
                ...
                <tr key={s._id}>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex justify-end">
                      <button type="button" aria-label={`Delete ${s.name}`} title={`Delete ${s.name}`} onClick={() => setDeleteTarget(s)} className="rounded-lg p-2 text-ink/50 hover:bg-clay-light hover:text-clay">
                        <Trash2 size={15} />
                      </button>
                    </div>
                  </td>
          ...
          <div>
            ...
            {facultyList.length === 0 ? (
              ...
              <div className="flex max-h-40 flex-col gap-1.5 overflow-y-auto rounded-xl border border-ink/15 p-2.5">
                {facultyList.map((f) => (
                  <label key={f._id} className="flex items-center gap-2.5 rounded-lg px-2 py-1.5 text-sm hover:bg-indigo/5">
                    <input
                      type="checkbox"
