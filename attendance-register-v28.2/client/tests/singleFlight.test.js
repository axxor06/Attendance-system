...
test('five concurrent expired-request callers trigger one refresh task', async () => {
  const gate = deferred();
  let calls = 0;
  const requests = Array.from({ length: 5 }, () => runSingleFlight(async () => {
    calls += 1;
    return gate.promise;
  }));
  ...
  assert.deepEqual(await Promise.all(requests), ['refreshed', 'refreshed', 'refreshed', 'refreshed', 'refreshed']);
});
